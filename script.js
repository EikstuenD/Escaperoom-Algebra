// --- KONFIGURASJON ---
const maxLevels = 5;
const tasksPerLevel = 12; // Økt fra 7 til 12 for lengre spilletid
let currentLevel = 1;
let tasksSolved = 0;
let difficulty = 1; 
let currentAnswer = 0;
let timeLeft = 1500; // Økt til 25 minutter (1500 sekunder)
let timerInterval;
let combo = 0;
let agentName = "AGENT";
...
const letters = "0101XYHAZK"; const fontSize = 16;
...
window.onload = function() {
    ...
    for(let i=0; i<16; i++){ let d = document.createElement('div'); d.className = 'matrix-char'; d.innerText = "0"; mg.appendChild(d); }
};
...
window.startGame = function(selectedDifficulty) {
    ...
    if (audioCtx.state === 'suspended') audioCtx.resume();
    startTimer(); updateUI(); updateLockVisuals(true); generateQuestion();
...
function generateQuestion() {
    ...
    let type = getNum(1, 4); // Velger tilfeldig oppgavetype 1-4
...
function generateDebugTask() {
    ...
    document.getElementById('question-text').innerHTML = logMsg;
    document.getElementById('question-text').style.color = "#ffaa00";
}

// --- HINT SYSTEM ---
window.useHint = function() {
    timeLeft -= 60; playSound('wrong'); 
    let hint = "";
    ...
    hDiv.innerText = "HINT: " + hint; hDiv.style.display = 'block';
}
...
window.checkAnswer = function() {
    ...
    if (userVal === currentAnswer) {
        ...
        if (combo >= 3) { document.body.classList.add('combo-mode'); document.getElementById('combo-badge').style.display = 'inline'; matrixColor = "#ffcc00"; }
        animateLock();
        if (tasksSolved >= tasksPerLevel) nextLevel();
        else setTimeout(generateQuestion, 1000);
    } else {
        playSound('wrong'); combo = 0; document.body.classList.remove('combo-mode'); document.getElementById('combo-badge').style.display = 'none'; matrixColor = "#00ff41";
        document.getElementById('user-input').classList.add('shake'); setTimeout(() => document.getElementById('user-input').classList.remove('shake'), 500);
    }
    document.getElementById('user-input').value = ""; document.getElementById('user-input').focus(); updateUI();
}
...
function nextLevel() {
    ...
    setTimeout(() => {
        currentLevel++; tasksSolved = 0; combo = 0; document.body.classList.remove('combo-mode'); matrixColor = "#00ff41";
        if (currentLevel > maxLevels) { document.getElementById('game-wrapper').style.display = 'none'; document.getElementById('victory').classList.add('active'); }
        else { updateLockVisuals(true); generateQuestion(); updateUI(); }
    }, 2000);
}

// --- VISUELLE EFFEKTER & LÅSER ---
function animateLock() {
    const ls = document.getElementById('lock-status-text'); ls.innerText = "DEKRYPTERER..."; ls.style.color = "yellow";
    if (currentLevel === 1) { 
        document.getElementById('keypad-screen').innerText = "CODE: " + currentAnswer; 
        let keys = document.querySelectorAll('.keypad-grid div'); keys[Math.floor(Math.random()*12)].classList.add('pressed'); 
        setTimeout(()=> document.querySelectorAll('.pressed').forEach(k=>k.classList.remove('pressed')), 200);
    }
    else if (currentLevel === 2) { rotation += (360/tasksPerLevel); document.getElementById('safe-dial').style.transform = `rotate(${rotation}deg)`; if(document.getElementById(`sl-${Math.ceil((tasksSolved/tasksPerLevel)*7)}`)) document.getElementById(`sl-${Math.ceil((tasksSolved/tasksPerLevel)*7)}`).classList.add('on'); }
    else if (currentLevel === 3) { if(document.getElementById(`srv-${Math.ceil((tasksSolved/tasksPerLevel)*7)}`)) document.getElementById(`srv-${Math.ceil((tasksSolved/tasksPerLevel)*7)}`).classList.add('hacked'); }
    else if (currentLevel === 4) { let chars = document.querySelectorAll('.matrix-char'); chars[Math.floor(Math.random()*chars.length)].classList.add('active'); }
    else if (currentLevel === 5) { let pct = Math.floor((tasksSolved/tasksPerLevel)*100); document.getElementById('core-center').innerText = pct + "%"; }
}

function updateLockVisuals(reset) {
    document.querySelectorAll('.lock-mechanism').forEach(el => el.classList.remove('active'));
    let active = document.getElementById(`lock-level-${currentLevel}`); if(active) active.classList.add('active');
    ...
    if(reset) {
        document.getElementById('lock-status-text').innerText = "LÅST"; document.getElementById('lock-status-text').style.color = "red";
...
function startFirewallLoop() {
    setInterval(() => { if (Math.random() < 0.25 && document.getElementById('firewall-modal').style.display !== 'flex') triggerFirewall(); }, 40000);
}
...
function triggerFirewall() {
    ...
    window.checkFirewall = function() { if (parseInt(input.value) === (n1 + n2)) { clearInterval(fwTimer); modal.style.display = 'none'; playSound('correct'); matrixColor = document.body.classList.contains('combo-mode') ? "#ffcc00" : "#00ff41"; document.getElementById('user-input').focus(); } else input.style.borderColor = "red"; }
}
...
function updateUI() { document.getElementById('sub-progress-text').innerText = `${tasksSolved}/${tasksPerLevel}`; document.getElementById('level-indicator').innerText = `Nivå: ${currentLevel}/${maxLevels}`; }
function startTimer() {
    timerInterval = setInterval(() => {
        let m = Math.floor(timeLeft / 60); let s = timeLeft % 60;
        document.getElementById('timer').innerText = `${m}:${s < 10 ? '0'+s : s}`;
        if (timeLeft <= 0) { clearInterval(timerInterval); alert(
